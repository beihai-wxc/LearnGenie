...
type NavKey = 'account' | SettingsSection;
...
export function SettingsDialog({ open, onOpenChange, initialSection }: SettingsDialogProps) {
  ...
  // Navigation
  const [activeSection, setActiveSection] = useState<NavKey>('account');
  ...
  const getHeaderContent = () => {
    if (activeSection === 'account') {
      return <h2 className="text-lg font-semibold">账户</h2>;
    }
    switch (activeSection as SettingsSection) {
      case 'general':
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="h-[85vh] max-w-3xl p-0 gap-0 block" showCloseButton={false}>
        ...
        <div className="flex h-full overflow-hidden">
          ...
          <div className="flex-shrink-0 bg-muted/30 p-3 space-y-1 flex flex-col" style={{ width: sidebarWidth }}>
            {/* Account tab */}
            <button
