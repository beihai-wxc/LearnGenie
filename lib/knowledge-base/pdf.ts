...
export function buildSimplePdf(title: string, body: string): Buffer {
  ...
  const textLines = [
    ...splitLines(title, 32).map(
      (line) => `BT /F1 20 Tf 50 780 Td <${toPdfHex(line)}> Tj ET`,
    ),
  ];
  ...
  for (const paragraph of safeBody.split('\n')) {
    ...
    for (const line of lines) {
      if (y < 60) break;
      textLines.push(`BT /F1 11 Tf 50 ${y} Td <${toPdfHex(line)}> Tj ET`);
      y -= 14;
    }
    y -= 8;
  }
  ...
  const objects = [
    ...
    '6 0 obj << /Type /Font /Subtype /CIDFontType0 /BaseFont /STSong-Light /CIDSystemInfo << /Registry (Adobe) /Ordering (GB1) /Supplement 4 >> /DW 1000 >> endobj',
  ];
